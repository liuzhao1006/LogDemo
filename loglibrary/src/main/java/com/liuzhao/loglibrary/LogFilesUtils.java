...
public class LogFilesUtils {
     ...
     *默认 log文件大小10MB
     * */
    private static final int DEFAULT_LOG_FILE_MAX_SIZE = 1024 * 1024 * 10;
     ...
     * */
    LogFilesUtils(String logFileDir, String appendFileName, int logFileCount, int logFileSize) {
        mLogFileDir = logFileDir;
    ...
    private File createNewLogFile() {
        return createFile(mLogFileDir + File.separator + DATE_FORMAT.format(new Date(System
                .currentTimeMillis())) + mAppendFileName + ".log");
    }
     ...
     */
    synchronized private boolean delete(File path) {
        if (null == path) {
            return true;
        }
...
//            long logfileLength = mCurrentLogFile.length();

            DataInputStream dataInputStream = new DataInputStream(new FileInputStream(mCurrentLogFile));
